...
%%   Data structure :  MDL   Model Data Created of Simulation Model  
         ...
         Plot_Active_Influence_Nodes_of_Gauss_Point( 28,DAT,MDL);  
...
%%--------------------------------------------------------              
     function [ DAT MDL ]= Compute_Active_Nodes_for_Gauss_Quad_Points(DAT,MDL) 
        ...
        fprintf('\n   Gauss Quad Points and their Active Nodes')
...
%%--------------------------------------------------------              
  ...
  %%  Nodes on the top of the beam surface
    ...
    %   Bottom surface of the beam                       
        ...
        fprintf('\n   Displ-Boudry Nodes lying on Beams Boundaries ')
...
%%--------------------------------------------------------              
     function [ DAT MDL ]= Compute_Shape_Functn_of_GausPt_Inside_Domain ...
                  (icell, igpt, DAT, MDL)  
             
            nANds =MDL.GPts_Active_nNodes(icell,igpt) 
            ANd(1:nANds,1)=MDL.GPts_Active_Node(icell,igpt,1:nANds)   
            xgpt= MDL.GausXCPt(icell, igpt)
            ygpt= MDL.GausYCPt(icell, igpt) 
            Wgpt= MDL.GausWCPt(icell, igpt)
            Jgpt= MDL.GausJCPt(icell, igpt)  
            nTerms= DAT.ShFn_nTerms
            Amtx(1:nTerms,1:nTerms)=0
            for ii=1: nANds
                node=ANd(ii)  
                xndpt=MDL.Node_XCord(node)
                yndpt=MDL.Node_XCord(node) 
                xdif=xgpt-xndpt;  ydif=ygpt-yndpt 
                Rad_Parm=sqrt(xdif^2+ydif^2)/MDL.Serch_Radius  
                [ Wght derWght] = Compute_Node_Shape_Weight(Rad_Parm)
                Pvec=[ 1 ; xndpt ; yndpt ] 
                Amtx= Amtx+ Wght*Pvec*Pvec' 
                Bmtx(1:nTerms,ii)=Wght*Pvec(1:nTerms,1)
            end    
%%------------------------------------------------------- 
%%       Compute_Shape_Functn_of_GausPt_Inside_Boudry
%%--------------------------------------------------------             
        function [ Wght derWght] = Compute_Node_Shape_Weight(Rad_Parm)
            if(Rad_Parm <= 0.5)
